...
│   │   ├── execution/
│   │   │   └── worker.ts        # Pyodide Web Worker runner
│   │   ├── supabase/
...
Python code runs entirely in the browser via [Pyodide](https://pyodide.org), a CPython port compiled to WebAssembly. Execution is isolated in a Web Worker (`src/lib/execution/worker.ts`) so it never blocks the UI thread.
...
**Review intervals:** NEEDS_REFRESH → 7d, IN_PROGRESS → 3d, recent MASTERED → 1d.
...
sequenceDiagram
    ...
    Hook->>Worker: "postMessage({ code, tests })"
    ...
    Worker-->>Hook: "postMessage({ results, stdout, stderr })"
...
Model IDs are centralized in `src/lib/ai/models.ts`. Swapping models requires changing a single constant, not touching prompt or streaming logic.
...
**Vercel integration:** Every PR gets a preview deployment. Merge to `main` triggers a production deployment automatically.
...
flowchart LR
    ...
    D -->|"Merged"| E["Vercel production deploy\nnew version live"]
```
